...
export class PumpMonitor extends EventEmitter {  
  ...
  // FIXED: Get token metadata without making API calls
  private async getTokenMetadata(tokenMint: string): Promise<TokenMetadata> {
    ...
    // For pump.fun tokens, we can use standard values without API calls
    // All pump.fun tokens have:
    // - 6 decimals
    ...
    try {
      ...
      if (result.rows.length > 0 && result.rows[0].symbol) {
        // We have metadata in DB, still use standard decimals/supply
    ...
    // Don't make any API calls here - let the metadata fetcher handle that separately
    return defaultMetadata;
  }

  // FIXED: Updated handleStreamData to ensure token is saved before processing price updates
    ...
    try {
      if (data.transaction) {
        ...
        if (newToken) {
          ...
          // Save token immediately instead of just queuing
          try {
            ...
            console.log(`✅ Token ${newToken.address.substring(0, 8)}... saved to database`);
            
            // Remove from creation queue since it's saved
            this.tokenCreationQueue.delete(newToken.address);
            this.knownTokens.add(newToken.address);
        ...
        if (parsedData) {
          ...
          if (priceUpdate) {
            // Check if token exists or is being created
            if (this.tokenCreationQueue.has(priceUpdate.token)) {
              console.log(`⏳ Token ${priceUpdate.token.substring(0, 8)}... still being created, skipping price update`);
              return;
            }
            
            // Verify token exists before buffering price update
            const tokenExists = await db.checkTokenExists(priceUpdate.token);
            if (!tokenExists) {
              console.warn(`⚠️ Token ${priceUpdate.token.substring(0, 8)}... not found in DB, creating placeholder`);
              
              // Create placeholder token entry
              try {
                await db.upsertToken({
  ...
  // FIXED: Updated flush method that ensures tokens exist before inserting price updates
  private async flush() {
        ...
        if (exists) {
          ...
          try {
            await db.upsertToken({
              ...
              bondingCurve: 'unknown', // We'll update this when we get the actual data
              symbol: null,
              name: null,
              imageUri: null,
              vanityId: null
