// src/monitor/index.ts - RESTORED ORIGINAL with only the bonding curve fix
...
export class PumpMonitor extends EventEmitter {    
  ...
  constructor() {    
    super();    
    // StreamClient now uses env vars from config  
  ...
  async start(): Promise<void> {    
    ...
    // ✅ CRITICAL: This was missing - start subscribing!
    this.streamClient.subscribe(subscription).catch(error => {  
  ...
  // ✅ ONLY CHANGE: Fixed this method to not create "unknown" placeholders
  private async handlePriceUpdate(priceUpdate: PriceUpdate): Promise<void> {    
    ...
    if (!tokenExists && !this.cacheService.isKnownToken(priceUpdate.token)) {    
      console.warn(`⚠️ Token ${formatTokenAddress(priceUpdate.token)} not found in DB, skipping price update`);    
      // ✅ FIXED: Skip instead of creating placeholder with 'unknown' bonding curve
      return;
    }
