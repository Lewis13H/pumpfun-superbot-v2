...
async function processAccountUpdate(data: any): Promise<void> {
  try {
    ...
    try {
      ...
      // Log pool update
      console.log(chalk.gray(`\n✓ Pool updated: ${poolData.quoteMint.slice(0, 8)}... | LP: ${poolData.lpSupply.toLocaleString()}`));
...
function displayStats() {
  ...
  console.log(`Updates: ${stats.accountUpdates.toLocaleString()} | Pools: ${stats.poolsTracked.size} | Rate: ${rate.toFixed(2)}/s`);
  console.log(`Decoded: ${stats.decodedPools.toLocaleString()} | Errors: ${stats.decodeErrors.toLocaleString()}`);
  console.log(chalk.gray('─'.repeat(50)));
}
